        ...
        .directive('myAnimalsDirective', function () {
            return {
                ...
                controller: function ($scope, $http) {
                    ...
                    $scope.ShowActivity = function (animal) {
                        $http.get(baseURL + "/cli_panel/activities/" + animal.id + "/animals.json").success(function (response) {
                            $scope.Activiteis = response;
                            $scope.CurrentAnimalSelected = animal.name;
                            if (response.length > 0) {
                                $("#ActivityModal").modal('show');
                            } else {
                                $("#ActivityError").modal('show');
                            }
...
//                    $scope.doModalAnimalsEdit = function(index, item) {
//                        //$scope.showModal = true;
//                        $scope.SelectedAnimalsTypeCategory = item.animalsTypeId;
//                        $scope.animalID = item.id;
//                        //$scope.photoEdit = photo;
//                        $scope.AnimalNameEdit = item.name;
//                        $scope.AnimalAgeEdit = item.age;
//                        $scope.AnimalStatureEdit = item.stature;
//                        $scope.AnimalMicroChipEdit = item.microChip;
//                        $scope.AnimalweightEdit = item.weight;
//                        $scope.AnimalSex = item.sex;
//                        $scope.selectCategoryAnimalsEdit = item.animalsTypeId;
//                        $scope.indexOfCurrentAnimalsEdit = index;
//                        $scope.file = item.file;
//                        $scope.dateHistory = item.dateHistory;
//                        $scope.Describ = item.Describ;
//                        $scope.file_id = item.file_id;
...
//                        });
//
//                        $http.get(baseURL + "/cli_panel/pictures/" + item.id + "/animals.json").success(function(response) {
...
//
//                    };
                    $scope.ShowHistory = function (animal) {
                        $scope.$broadcast("History", [animal]);
                        $('#ShowHistory').modal();
                    };
            ...
            return {
                ...
                controller: function ($scope, $http) {
                    ...
                    $scope.ShowMessage = function (index, idMessage) {
                        ...
                        $scope.messageUser[index].is_read = 1;
            ...
            return {
                ...
                controller: function ($scope, $http, $cookies) {
                    ...
                    $http.get(baseURL + "/cli_panel/user/mojodi/hesab.json").success(function (response) {
                        if (response.monyUser == undefined) {
                            $scope.mojodiHesab = 0;
                        } else {
...
}).directive('inputfileanimale', function ($routeParams, $compile) {
    return {
        template: '<input type="file"  class="form-control"   />',
        replace: true,
        scope: false,
...
}).directive('inputfileparvande', function ($routeParams, $compile) {
    return {
        ...
        restrict: 'E',
        link: function ($scope, elm, attrs) {
            $compile($('.AddFileParvande').html("<img class='pull-left' ng-click='editeImage2($event)' ng-src='{{file}}' style='max-width:100%' />"))($scope);
            elm.change(function (evt) {
                var file = evt.currentTarget.files[0];
...
//                            console.log($scope.file)
        ...
        controller: function ($scope, $element) {
            $scope.editeImage2 = function (event) {
                console.log($(event.target).before());
                $(event.target).before().val('/');
                $scope.imageLoadForEdit2($scope.file);
                ...
                image.onload = function () {
                    ...
                    $('#submit').on('click', function (e) {
                        ...
                        $scope.$apply(function ($scope) {
                            ...
                            $scope.file = $(cropper).cropper('getCroppedCanvas', {
                                width: 640,
                                height: 480
                            }).toDataURL("image/jpeg", 0.5);
                            $('#cropper').hide();
...
app.directive('history', function () {
    ...
    directive.restrict = 'E',
            directive.templateUrl = '../../bundles/app/partials/History.html';
    return directive;
});
...
app.directive('historyshow', function () {
    ...
    show.restrict = 'E';
    show.scope = false;
    show.templateUrl = '../../bundles/app/partials/History/Show.html';
    return show;
});
