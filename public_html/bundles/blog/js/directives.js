...
//                                console.log(height2);
        ...
        directive('showentity', function($routeParams, $compile) {
            return {
                template: '<span /></span>',
                replace: true,
                scope: false,
                link: function($scope, elm, attrs) {
                    var show = entity[$routeParams.entity]['show'][parseInt(attrs.index)];
                    elm.html($compile(' <strong> {{selectedItem.' + show.name + '.' + show.prop + '}}</strong>')($scope));
                    elm.removeAttr('index');
                }
            };
        }).
        ...
        directive('showtext', function($routeParams, $compile) {
            return {
                ...
                link: function($scope, elm, attrs) {
                    var show = entity[$routeParams.entity]['show'][parseInt(attrs.index)];
                    elm.html($compile('<div style="max-width:99%;max-height:300px;overflow-y:scroll;box-shadow: 0 0 1px;padding: 20px;border: 1px solid #2B579A;border-bottom: 27px solid #2B579A;" bind-html-unsafe="selectedItem.' + show.name + '"></div><br>')($scope));
                    elm.removeAttr('index');
                }
            };
        ...
        }).directive('animale', function() {
    return {
        restrict: 'E',
        scope: false,
        templateUrl: BaseUrl + "bundles/blog/partials/admin/Index/Animale.html"
    };
});